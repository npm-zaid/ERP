...
const SalesInvoiceAddWindow = ({ newEntry, setNewEntry, handleModalSubmit, modal, isEditMode }) => {
  ...
  const rowFields = [
    { field: 'delDate', label: 'Del. Date', type: 'text', icon: 'calendar' },
    ...
    { field: 'lrFreight', label: 'L.R. Freight', type: 'number' },
    { field: 'bc', label: 'BC', type: 'number' },
    { field: 'hamali', label: 'Hamali', type: 'number' },
    { field: 'otherCharge', label: 'Other Charge', type: 'number' },
    { field: 'subTotal', label: 'Sub Total', type: 'number', readOnly: true },
    { field: 'gst', label: 'GST', type: 'number', readOnly: true },
    { field: 'totalFreight', label: 'Total Freight', type: 'number', readOnly: true },
    { field: 'fromBranch', label: 'From Branch', type: 'dropdown', options: [
      'MUMBAI',
      'AHMEDABAD',
      'DELHI',
      'PUNE',
  ...
  const updateCalculations = (updatedEntry) => {
    ...
    const updatedRows = rows.map(row => {
      const lrFreight = parseFloat(row.lrFreight) || 0;
      const bc = parseFloat(row.bc) || 0;
      const hamali = parseFloat(row.hamali) || 0;
      const otherCharge = parseFloat(row.otherCharge) || 0;
      const subTotal = lrFreight + bc + hamali + otherCharge;
  ...
  return (
    <div className="text-sm">
      ...
      <div className="bg-gray-200/60 p-2 rounded-lg mb-4">
        <h3 className="text-[13px] font-semibold text-gray-700 mb-2">Invoice Details</h3>
        <div className="grid grid-cols-5 gap-3">
          ...
          <div>
            {renderInputField('invtype', newEntry.invtype, {
              type: 'dropdown',
      ...
      {/* Section 2: Additional Invoice Info */}
      <div className="bg-gray-200/60 p-2 rounded-lg mb-4">
        <h3 className="text-[13px] font-semibold text-gray-700 mb-2">Additional Invoice Info</h3>
        <div className="grid grid-cols-5 gap-3">
          <div>
...
export default SalesInvoiceAddWindow;